...
def navigate_to_google_maps(driver, query):  # 搜尋
    try:
        ...
        search_element = driver.find_element(
            By.CSS_SELECTOR, "input#searchboxinput.searchboxinput.xiQnY"
        )
...
def scroll_page(driver):  # 滾動
    try:
        target_element = driver.find_element(
            By.XPATH,
            '//*[@class="m6QErb DxyBCb kA9KIf dS8AEf ecceSd" and @role="feed"]',
        )
        driver.execute_script(
            "arguments[0].scrollTo({ top: arguments[0].scrollTop + 200, behavior: 'smooth' });",
            target_element,
        )
        sleep(0.5)
        innerHeight = 0
        ...
        while count <= limit:
            ...
            target_div = driver.find_element(
                By.XPATH, '//*[@class="m6QErb DxyBCb kA9KIf dS8AEf ecceSd QjC7t" ]'
            )
...
def extract_attractions_info(driver):  # 抓景點名 href 全部有幾星跟幾條評論 (第一層爬蟲)
    try:
        ...
        hrefs = [
            ...
            for element in driver.find_elements(By.CSS_SELECTOR, "a.hfpxzc")
        ]

        star_and_comments = []
        for elements in driver.find_elements(
            By.CSS_SELECTOR, "span.e4rVHe.fontBodyMedium"
        ):
        ...
        try:
            ...
            try:
                area_element = driver.find_element(
                    By.XPATH, '//div[@class="Io6YTe fontBodyMedium kR99db "]'
                )
            ...
            try:
                time_element = driver.find_element(
                    By.XPATH, '//div[@class="t39EBf GUrTXd"]'
                )
            ...
            try:
                elements = driver.find_elements(
                    By.CSS_SELECTOR, "tr.BHOKXe[aria-label]"
                )
                merged_star = "".join(
                    [element.get_attribute("aria-label") + ", " for element in elements]
                ).rstrip(", ")
            ...
            try:
                tags = driver.find_elements(
                    By.CSS_SELECTOR, "span.uEubGf.fontBodyMedium"
                )
    ...
    try:
        ...
        with open(list_path, "r", encoding="utf-8") as clocation:
            ...
            for line in lines:
                ...
                areas, times, stars, tag = extract_additional_info_stars_and_tags(
                    driver, hrefs
                )
                attractions = [
                    info + (area, time, stars, tag)
                    for info, area, time, stars, tag in zip(
                        attractions_info, areas, times, stars, tag
                    )
                ]
