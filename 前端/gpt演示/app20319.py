...
mongo_uri = "mongodb+srv://root:Anny12345!@cluster0.sbvqogz.mongodb.net/?retryWrites=true&w=majority"
# client = MongoClient(mongo_uri)
# try:
if True:
    # test = MongoClient(mongo_uri, ssl=True,ssl_cert_reqs='CERT_NONE')
    print("0")
    test = MongoClient(mongo_uri, tlsCAFile=certifi.where())
    print("1")
    print(f"1+{test}")
    # db = client["Taiwan_travel"]
    db = test.get_database("Taiwan_travel")
    print("2")
    print(f"2+{db}")
    collection = db["data"]
    print("3")
    print(f"3+{collection}")
    print("成功連接到 MongoDB！")
# except Exception as e:
#     print("無法連接到 MongoDB：", e)

try:
    test.root.command("ping")
...
llm = ChatOpenAI(api_key="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")
...
def start_response():
    ...
    StartResponse = "我為您精選了一些不可錯過的景點與活動。以下是關於您感興趣的目的地的一些精彩介紹，以及我個人推薦的行程安排：\n"
    return StartResponse


def end_response():
    # 同上，调整为返回一个字符串
    EndResponse = "希望以上的資訊能夠幫助您規劃一次難忘的旅行。"
    return EndResponse
...
def bii():
    ...
    return render_template("bii.html")
...
def submit():
    ...
    if request.method == "POST":
        ...
        final_response = " ".join(str(itinerary) for itinerary in final_itinerary)
...
@app.route("/api/show_places", methods=["GET", "POST"])
def show_places():
    global session
    if request.method == "POST":
        ...
        print(places_info)
        # return render_template("index.html", places=places_info)
        return places_info
